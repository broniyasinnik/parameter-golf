...
# Tensors matching these patterns get int8 (gentler) instead of int6.
# Embeddings aren't protected by STE during training, so they need the wider range.
INT8_FULL_RANGE_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get("INT8_FULL_RANGE_PATTERNS", "tok_emb").split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_xsa = False  # set by GPT.__init__ for selected layers

    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        """Subtract self-value projection via GQA-aware reshape (zero allocation).
